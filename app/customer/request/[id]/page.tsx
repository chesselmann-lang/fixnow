...
export default async function RequestDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: offers } = await supabase
    .from('offers')
    .select('*, provider:provider_profiles(rating_avg, rating_count, bio, profile:profiles(full_name, avatar_url))')
    .eq('request_id', id)
  ...
  return (
    <div>
        ...
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <div className="flex flex-wrap gap-2 mt-2">
              ...
              <span className="text-xs bg-orange-50 text-orange-600 px-2 py-1 rounded-full">{URGENCY_LABELS[request.urgency]}</span>
            </div>
          </div>
      ...
      {/* Chat mit akzeptiertem Dienstleister */}
      {acceptedOffer && (
        <div className="mb-5">
          <h2 className="text-base font-bold text-gray-800 mb-2 flex items-center gap-2">
            <MessageSquare size={16} className="text-orange-500" />
            Chat mit {acceptedOffer.provider?.profile?.full_name ?? 'Dienstleister'}
          </h2>
              ...
              >
                {offer.status === 'accepted' && (
                  <div className="flex items-center gap-1 text-green-600 text-xs font-semibold mb-3">
                    <CheckCircle2 size={14} /> Akzeptiertes Angebot
                  </div>
                )}
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <p className="font-semibold text-gray-800 text-sm">
                          {offer.provider?.profile?.full_name ?? 'Dienstleister'}
                        </p>
                        {offer.provider?.rating_count > 0 && (
                          <div className="flex items-center gap-1 text-xs text-gray-400">
                            <Star size={10} className="text-yellow-400 fill-yellow-400" />
                            {Number(offer.provider.rating_avg).toFixed(1)} ({offer.provider.rating_count} Bewertungen)
                          </div>
                        )}
                      </div>
                    </div>
